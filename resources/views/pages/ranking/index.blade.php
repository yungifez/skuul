...
@php
    $chosen = $section !== null || $cohort !== null;
    $groupName = $cohort?->name ?? $section?->name;
@endphp
...
@section('content')
    <div class="space-y-6">
        ...
        <april:card>
            <slot:title>Choose a group</slot:title>
            <slot:description>
            ...
            <slot:content>
                <form method="GET" action="{{ route('rankings.index') }}" class="grid gap-4 lg:grid-cols-5 lg:items-end">
                    ...
                    <div class="flex flex-col gap-2">
                        <april:label for="cohort_id">Or a group</april:label>
                        ...
                        <p class="text-xs text-muted-foreground">A group wins when both are chosen.</p>
                    </div>

                    <div class="flex flex-col gap-2">
                        <april:label for="academic_period_id">{{ school_term('period', 'Period') }}</april:label>
                        <april:native-select id="academic_period_id" name="academic_period_id">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <april:native-select id="course_offering_id" name="course_offering_id">
                            <option value="">Every subject</option>
                            @foreach ($offerings as $option)
                                <option value="{{ $option->id }}" @selected($offering?->id === $option->id)>{{ $option->subject?->name ?? 'Unnamed' }}</option>
                            @endforeach
                        </april:native-select>
                    </div>
