@php
    ...
    $selectedSubjectId = (string) old('subject_id', request()->integer('subject_id') ?: '');
...
@extends('layouts.app', ['breadcrumbs' => [
    ...
    ['href' => route('course-offerings.bulk-create.form', ['academic_year_id' => $selectedAcademicYear->id]), 'text' => 'Set up across levels', 'active'],
]])
            ...
            <slot:content>
                <form method="POST" action="{{ route('course-offerings.bulk-store') }}" class="space-y-8" x-data="{ selectedLevels: @js($selectedLevelIds) }">
                    ...
                    <div class="flex flex-wrap items-center gap-3 border-t pt-6">
                        ...
                        <april:button-link href="{{ request()->boolean('setup') ? route('course-offerings.bulk-create', ['academic_year_id' => $selectedAcademicYear->id, 'setup' => 1]) : route('course-offerings.index') }}" variant="ghost">Cancel</april:button-link>
                    </div>
