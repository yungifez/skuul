...
<ol class="capitalize my-4">
    @isset ($paths)
        <i class="fa fa-home mr-2 text-sm" aria-hidden="true"></i>
        @foreach ($paths as $path)
            @if (!in_array('active', $path))
                <li class="breadcrumb-item inline mx-1 text-capitalize text-sm"><a class="text-blue-600" href="{{$path['href']}}">{{__($path['text'])}}</a></li>
            @else
