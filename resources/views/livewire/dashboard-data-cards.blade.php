<div class="space-y-8">
    @if (auth()->user()->can(\App\Enums\PlatformPermission::AccessAllSchools) || $isOrganizationAdministrator || auth()->user()->hasRole(\App\Enums\Role::Admin))
        ...
        @php
            $snapshotStats = [
                ['label' => school_terms('class_level', school_term('class_level', 'Grade or class level')), 'value' => $academicLevels, 'icon' => 'presentation', 'permission' => 'read class', 'href' => route('academic-levels.index'), 'description' => 'Levels'],
                ['label' => school_terms('section', school_term('section', 'Class')).' this year', 'value' => $cycleSections, 'icon' => 'landmark', 'permission' => 'read section', 'href' => route('academic-cycle-sections.index'), 'description' => 'Active groups'],
                ['label' => school_terms('period', school_term('period', 'Term or reporting period')), 'value' => $academicPeriods, 'icon' => 'clock', 'permission' => 'read academic period', 'href' => route('academic-periods.index'), 'description' => 'Open periods'],
