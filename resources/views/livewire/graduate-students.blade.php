<div class="card">
    ...
    <div class="card-body">
        ...
        <form wire:submit.prevent="loadStudents" class="md:grid grid-cols-2 gap-4">
            <x-select id="class" name="class" label="Class" wire:model="class" fgroup-class="col-md-6">
                @foreach ($classes as $class)
                    <option value="{{$class['id']}}">{{$class['name']}}</option>
                @endforeach
            </x-select>
            <x-select id="section" name="section" label="Section" wire:init="loadInitialSections" wire:model="section" fgroup-class="col-md-6">
                @isset($sections)
