            ...
            <slot:content class="space-y-4">
                @forelse ($campuses as $campus)
                    ...
                    <div wire:key="campus-health-{{ $campus->id }}" class="grid gap-4 rounded-lg border p-4 lg:grid-cols-[minmax(12rem,1.4fr)_repeat(4,minmax(0,1fr))_auto] lg:items-center">
                        ...
                        <div><p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Semester</p><p class="mt-1 font-semibold">{{ $campus->semester?->name ?? 'Not set' }}</p><p class="text-sm text-muted-foreground">{{ $this->academicPeriodStatus($campus->semester?->status) }}</p></div>
