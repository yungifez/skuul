    ...
    <div class="card-body">
        ...
        <div class="md:flex justify-between my-3 parent-horizontal-align">
            ...
            <div class="py-3 border-t border-b horizontal-align">
                ...
                <p>{{$feeInvoice->name}}</p>
                <p>Current {{ strtolower(school_term('section', 'section')) }}: {{ $feeInvoice->studentRecord?->academicCycleSection?->academicLevel?->name ?? 'Not currently placed' }}@if ($feeInvoice->studentRecord?->academicCycleSection) · {{ $feeInvoice->studentRecord->academicCycleSection->label ?? $feeInvoice->studentRecord->academicCycleSection->name }}@endif</p>
                <p>Student admission number: {{ $feeInvoice->studentRecord?->admission_number ?? 'Not recorded' }}</p>
            </div>
        </div>
