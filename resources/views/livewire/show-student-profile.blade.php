<div class="space-y-6">
        ...
        <slot:content class="space-y-6">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-lg border bg-muted/30 p-4">
                    ...
                    <p class="mt-1 font-semibold">{{ $studentRecord->admission_date?->format('M j, Y') ?: 'Not recorded'
                        }}</p>
                </div>
                <div class="rounded-lg border bg-muted/30 p-4">
                    <p class="text-xs font-medium uppercase text-muted-foreground">{{ school_term('class_level',
                        'Class') }}</p>
                    <p class="mt-1 font-semibold">{{ $studentRecord->academicCycleSection?->academicLevel?->name ?? 'Not
                        placed' }}</p>
                </div>
                <div class="rounded-lg border bg-muted/30 p-4">
                    <p class="text-xs font-medium uppercase text-muted-foreground">{{ school_term('section', 'Section')
                        }}</p>
                    <p class="mt-1 font-semibold">{{ $studentRecord->academicCycleSection?->label ??
                        $studentRecord->academicCycleSection?->name ?? 'Not placed' }}</p>
                </div>
            </div>
            ...
            <div class="grid gap-6 border-t pt-6 lg:grid-cols-2">
                <form wire:submit="changeStatus" class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">Every change is recorded with the actor, date, and
                            reason.</p>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <april:input-group id="status-effective-on" type="date" label="Effective on"
                            wire:model.live="statusEffectiveOn" />
                        <div class="flex flex-col gap-2">
                            <april:label for="status-reason">Reason</april:label>
                            <april:textarea id="status-reason" wire:model.live="statusReason" rows="2"
                ...
                <form wire:submit="changePlacement" class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">A new placement keeps the previous {{
                            strtolower(school_term('class_level', 'class')) }} and {{ strtolower(school_term('section',
                            'section')) }} in history.</p>
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select id="placement-cycle-section" wire:model.live="placementCycleSectionId"
                            class="h-10 rounded-md border border-input bg-background px-3 text-sm" {{ !$academicYear ||
                            $studentRecord->status->isClosed() ? 'disabled' : '' }} {{
                            field_error_bindings('placementCycleSectionId') }}>
                            <option value="">Choose a {{ strtolower(school_term('section', 'section')) }}</option>
                            @foreach ($cycleSections as $cycleSection)
                            <option value="{{ $cycleSection['id'] }}">{{ $cycleSection['level'] }} · {{
                                $cycleSection['name'] }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <april:input-group id="placement-effective-on" type="date" label="Effective on"
                            wire:model.live="placementEffectiveOn" />
                        <div class="flex flex-col gap-2">
                            <april:label for="placement-reason">Reason</april:label>
                            <april:textarea id="placement-reason" wire:model.live="placementReason" rows="2"
                                placeholder="Optional reason" />
                        </div>
                    </div>

                    <april:button type="submit" wire:loading.attr="disabled" wire:target="changePlacement"
                        :disabled="!$academicYear || $studentRecord->status->isClosed()">
                ...
                <form wire:submit="moveCampus" class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">Campuses of one organization share a student. The
                            enrollment, the admission number, and the placement history move with them. Use a transfer
                            only for another organization.</p>
                        ...
                        @else
                        <p class="mt-1 text-sm text-muted-foreground">The receiving campus has to agree. Your request
                            waits for their decision.</p>
                        @endif
                    </div>
                    ...
                    <april:alert>
                        ...
                        <slot:description>
                            ...
                            into {{ $openCampusMoveRequest->academicCycleSection->label ??
                            $openCampusMoveRequest->academicCycleSection->name }}
                            @endif
                            on {{ $openCampusMoveRequest->effective_on?->format('M j, Y') }}.
                        </slot:description>
                    </april:alert>
                    <april:button type="button" variant="destructive" wire:click="cancelCampusMove"
                        wire:loading.attr="disabled" wire:target="cancelCampusMove">
                    ...
                    @if (!$openCampusMoveRequest)
                    <div class="flex flex-col gap-2">
                        <april:label for="campus-cycle-section">Campus and {{ strtolower(school_term('section',
                            'section')) }}</april:label>
                        <select id="campus-cycle-section" wire:model.live="campusCycleSectionId"
                            class="h-10 rounded-md border border-input bg-background px-3 text-sm" {{
                            $studentRecord->status->isClosed() ? 'disabled' : '' }} {{
                            field_error_bindings('campusCycleSectionId') }}>
                            <option value="">Choose a campus {{ strtolower(school_term('section', 'section')) }}
                            </option>
                            @foreach ($campusCycleSections as $campusCycleSection)
                            <option value="{{ $campusCycleSection['id'] }}">{{ $campusCycleSection['campus'] }} · {{
                                $campusCycleSection['level'] }} · {{ $campusCycleSection['name'] }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <april:input-group id="campus-effective-on" type="date" label="Effective on"
                            wire:model.live="campusEffectiveOn" />
                        <div class="flex flex-col gap-2">
                            <april:label for="campus-reason">Reason</april:label>
                            <april:textarea id="campus-reason" wire:model.live="campusReason" rows="2"
                                placeholder="Optional reason" />
                        </div>
                    </div>

                    <april:button type="submit" wire:loading.attr="disabled" wire:target="moveCampus"
                        :disabled="$studentRecord->status->isClosed()">
    ...
    <div class="grid gap-6 lg:grid-cols-2">
                    ...
                    <div class="flex gap-3" wire:key="enrollment-status-change-{{ $change->id }}">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="text-sm text-muted-foreground">{{ $change->effective_on?->format('M j, Y')
                                    }}</span>
                            </div>
        ...
        <april:card>
            ...
            <slot:description>Every {{ strtolower(school_term('class_level', 'class')) }} and {{
                strtolower(school_term('section', 'section')) }} assignment for this enrollment.</slot:description>
            <slot:content>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y">
                            ...
                            <tr wire:key="enrollment-placement-{{ $placement->id }}">
                                <td class="px-2 py-3">{{ $placement->academicYear?->name ?: 'Not recorded' }}<span
                                        class="block text-xs text-muted-foreground">{{ $placement->academicPeriod?->name
                                        ?: 'No reporting period' }}</span></td>
                                <td class="px-2 py-3">{{ $placement->academicCycleSection?->academicLevel?->name ?? '—'
                                    }}<span class="block text-xs text-muted-foreground">{{
                                        $placement->academicCycleSection?->label ??
                                        $placement->academicCycleSection?->name ?? '—' }}</span></td>
                                <td class="whitespace-nowrap px-2 py-3">{{ $placement->effective_on?->format('M j, Y')
                                    }}</td>
                            </tr>
                            @endforeach
                        </tbody>
    ...
    <april:alert variant="destructive">
        ...
        <slot:description>This person has a user account but no student enrollment attached to the current school.
        </slot:description>
    </april:alert>
    @endif
</div>
