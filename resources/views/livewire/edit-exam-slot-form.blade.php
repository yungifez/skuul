<div class="card">
    ...
    <div class="card-body">
        <form action="{{route('exam-slots.update',[ $exam->id, $examSlot->id])}}" method="post" class="w-1/2">
            ...
            <x-input id="name" name="name" label="Exam slot Name" placeholder="Enter Exam slot name"  t->name}}"/>
            <x-textarea id="description" name="description" label="Description" placeholder="Enter description"  
                {{$examSlot->description}}
            </x-textarea>
            <x-input id="total_marks" name="total_marks" label="Maximum marks obtainable" placeholder="Enter max mark"  type="number" t->total_marks}}"/>
            @csrf
            @method('PUT')
